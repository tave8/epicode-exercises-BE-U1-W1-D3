...
public class SIM {
    ...
    public void increaseCredito(double amount) {
        if (amount < 0) {
            System.out.println("quantità del credito non può essere negativa");
        } else {
            this.credito += amount;
        }
    }

    public void decreaseCredito(double amount) {
        double newCredito = this.getCredito() - amount;
        if (amount > 0) {
            System.out.println("quantità del credito non può essere positiva");
